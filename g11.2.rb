...
#              E N D   P L A Y E R   C L A S S



#
#    D R O N E    C L A S S      D D D D D D D D D D D D D D D D D D D
#
class Drone
...
#
#    D R O N E    2    C L A S S      D 2 D 2 D 2 D 2 D 2 D 2 D 2 D 2 D
#
class Drone2
...
#
#   W I N D O W     C L A S S    W     W     W     W     W    W     W   W
#
...
#    @drone_create = Gosu::Image::draw_rot(@x, @y, ZOrder::Drone, @angle)
  ...
  def update
    ...
    if @drones.size < 1 then
      @drones.push(Drone.new(self, @drone_img, rand(1200), rand(800), ZOrder::Drone, 0.45))
    end

    if @drones2.size < 1 then
      @drones2.push(Drone2.new(self, @drone_img2, rand(1200), rand(800), ZOrder::Drone, 0.45))
    end
    ...
    if @drones.size + @drones2.size <= 780 then
        ...
        if rand(30) == 5 and drone.score >= 1500 then
          ...
          if rand(2) == 1 then
            @drones.push(Drone.new(self, @drone_img, drone.x, drone.y, ZOrder::SubDrone, \
              ((drone.veloz + rand(100) / 500.00) * 1.12)))
            else
              @drones.push(Drone.new(self, @drone_img, drone.x, drone.y, ZOrder::SubDrone, \
                ((drone.veloz - rand(100) / 500.00 ) * 0.88)))
          end
        end
      end
      ...
      @drones2.each do |drone|
        if rand(30) == 5 and drone.score >= 1500 then
          drone.score_reset
          if rand(2) == 1 then
            @drones.push(Drone2.new(self, @drone_img2, drone.x, drone.y, ZOrder::SubDrone, \
              ((drone.veloz + rand(100) / 500.00) * 1.12)))
            else
              @drones.push(Drone2.new(self, @drone_img2, drone.x, drone.y, ZOrder::SubDrone, \
                ((drone.veloz - rand(100) / 500.00 ) * 0.88)))
...
#
    ...
    if @drones.size < 4 and @stars.size < 150 then
      @stars.push(Star.new(@star_anim))
    end

    if rand(120) <= 2 and @stars.size < 150 then
      @stars.push(Star.new(@star_anim))
    end
  ...
  def draw
    ...
    @drones2.each { |drone2| drone2.draw }
    @stars.each { |star| star.draw }    
    @background_image.draw(0, 0, ZOrder::Background)
    @font.draw("PLAYER: #{@player.score}       #{@tot_score}          Num Drones: #{@drones.size}      Num Stars: #{@stars.size}", 10, 10, ZOrder::UI, 1.0, 1.0, 0xffffff00)
