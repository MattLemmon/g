...
#    @drone_create = Gosu::Image::draw_rot(@x, @y, ZOrder::Drone, @angle)
  ...
  def update
    ...
    if @drones.size < 5 and @drones2.size < 10 then
      @drones.push(Drone.new(self, @drone_img, rand(1200), rand(800), ZOrder::Drone, 0.70))
    end

    if @drones2.size < 5 and @drones.size < 10 then
      @drones2.push(Drone.new(self, @drone_img2, rand(1200), rand(800), ZOrder::Drone, 0.70))
    end
    ...
    @drones.each do |drone|
      if rand(30) == 5 and drone.food > 1500 then
        drone.food_reset
        if rand(2) == 1 then
          ...
          @drones.push(Drone.new(self, @drone_img, drone.x, drone.y, ZOrder::SubDrone, \
            ((drone.veloz - rand(100) / 400.00 ) * 0.90)))
        end
      end
    end
...
#
    ...
    if @drones.size < 8 and @drones2.size < 8 and @stars.size < 90 then
