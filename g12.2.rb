...
#######  M A T I N G  ##########
  ...
  def reproduce(drones2)
    drones2.each do |drone|
      if Gosu::distance(@x, @y, drone.x, drone.y) < 100
      @mate = 1
      else
      @mate = 0
      end
...
class GameWindow < Gosu::Window
  def initialize
    ...
    @drone_img = Gosu::Image.new(self, "media/Drone1.3.bmp", false)
    @drones = Array.new
#    @drone_create = Gosu::Image::draw_rot(@x, @y, ZOrder::Drone, @angle)
    @drone_img2 = Gosu::Image.new(self, "media/Drone1.4.bmp", false)
    @drones2 = Array.new
  ...
  def update
    ...
    if ( @drones.size < 10 and @drones2.size < 15 ) or ( @drones2.size < 10 and @drones.size < 15 ) then
...
#     and rand(70) <= 2 then
      if drone.mate == 1 and drone.mate_scr >= 400 and @drones.size < 700 then
        drone.mate_scr_rst
        if rand(4) == 1 then
...
#######  P U S H   S T A R S   ############
    if @drones.size < 4 and @drones2.size < 4 and @stars.size < 30 then
      @stars.push(Star.new(@star_anim))
    end

    if rand(100) <= 6 and @stars.size < 150 then
      @stars.push(Star.new(@star_anim))
    end
