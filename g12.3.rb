...
#######  M A T I N G  ##########
  ...
  def reproduce(drones2)
    drones2.each do |drone|
      if Gosu::distance(@x, @y, drone.x, drone.y) < 150
      @mate = 1
      else
      @mate = 0
      end
...
class GameWindow < Gosu::Window
  def initialize
    ...
    @drone_img = Gosu::Image.new(self, "media/Drone1.4.bmp", false)
    @drones = Array.new
...
#    end
############   P U S H    D R O N E S   #################################
    if ( @drones.size < 4 and @drones2.size < 15 ) or ( @drones2.size < 4 and @drones.size < 15 ) then
...
#     and rand(70) <= 2 then
      if drone.mate == 1 and drone.mate_scr >= 800 and @drones.size < 600 then
        drone.mate_scr_rst
        if rand(4) == 1 then
...
#######  P U S H   S T A R S   ############
    if @drones.size < 15 and @drones2.size < 15 and @stars.size < 100 then
      @stars.push(Star.new(@star_anim))
    end

    if rand(100) <= 20 and @stars.size < 300 then
      @stars.push(Star.new(@star_anim))
    end
