...
#    @drone_create = Gosu::Image::draw_rot(@x, @y, ZOrder::Drone, @angle)
  ...
  def update
    ...
    if @drones.size < 1 then
      @drones.push(Drone.new(self, @drone_img, rand(1200), rand(800), ZOrder::Drone, 0.85))
    end
...
#
    ...
    if @drones.size < 5 and @stars.size < 150 then
      @stars.push(Star.new(@star_anim))
    end

    if rand(140) <= 3 and @stars.size < 200 then
      @stars.push(Star.new(@star_anim))
    end
