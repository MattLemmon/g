...
=end
  ...
  def kill_drones(drones)
    drones.reject! do |drone|
      if Gosu::distance(@x, @y, drone.x, drone.y) < 80 then
        @beep.play
        @score += 1
        true
      else
...
#    @player.collect_stars(@stars)
    ...
    if @drones.size < 1 then
      @drones.push(Drone.new(self, @drone_img, rand(1200), rand(800), ZOrder::Drone, 0.75, 2.5, 2.5, 50, 50, 70))
    end
    ...
    @drones.each do |drone|
      if rand(30) == 5 and drone.score >= 4500 then
        drone.score_reset
