...
class Drone
  ...
  def collect_stars(stars)
    stars.reject! do |star|
      if Gosu::distance(@x, @y, star.x, star.y) < 70 then
        @score += 1000
        true
      else
        false
...
class Drone2
  ...
  def collect_stars(stars)
    stars.reject! do |star|
      if Gosu::distance(@x, @y, star.x, star.y) < 90 then
        @score += 1000
        true
      else
        false
...
#
  ...
  def draw
    ...
    @font.draw("      PLAYER : #{@player.score}       DRONES : #{totalScore}            Drone 1 : #{@drone.score}   Drone 2 : #{@drone2.score}   Drone 3 : #{@drone3.score}", 10, 10, ZOrder::UI, 1.0, 1.0, 0xffffff00)
